...
public class CommentOnMapEntriesTest extends TestCase {
	...
	/**
	 * Tests dumping with comments.
	 */
	public static void testDump() {
		final Yaml yml = new Yaml(new MyRepresenter());
		final MyCommentMap obj = new MyCommentMap();
		obj.mapLevelComments = new String[] { "# this is a top level comment." };
		final MyCommentMap foo = new MyCommentMap();
		obj.put("foo", foo);
		foo.mapLevelComments = new String[] { "# some nested comment" };
		foo.put("bar", Integer.valueOf(123));
		foo.valueComments.put("baz", new String[] { "# second nested comment" }); 
		foo.put("baz", Integer.valueOf(234));

		assertEquals(YAML_STRING, yml.dump(obj));
	}
